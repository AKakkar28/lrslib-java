...
/**
 * Immutable arbitrary-precision rational number.
 * - Always reduced (gcd-normalized)
 * - Denominator always positive
 * - Optimized cross-cancel arithmetic to reduce growth
 *
 * Mirrors lrslib’s exact rational arithmetic.
 */
public final class Fraction implements Numeric<Fraction>, Comparable<Fraction> {
    ...
    public static final Fraction NEG_ONE = new Fraction(BigInteger.ONE.negate(), BigInteger.ONE);

    private final BigInteger n; // numerator
    private final BigInteger d; // denominator > 0

    /** Creates reduced fraction with nonzero denominator. */
    ...
    // ---- Factories ----
    public static Fraction of(long k) {
        return new Fraction(BigInteger.valueOf(k), BigInteger.ONE);
    }

    public static Fraction of(long num, long den) {
        return new Fraction(BigInteger.valueOf(num), BigInteger.valueOf(den));
    }

    public static Fraction of(BigInteger k) {
        return new Fraction(k, BigInteger.ONE);
    }
    ...
    public static Fraction parse(String s) {
        ...
        int slash = t.indexOf('/');
        if (slash < 0) {
            return new Fraction(new BigInteger(t), BigInteger.ONE);
        }
    ...
    public Fraction divide(Fraction o) {
        ...
        // (n/d)/(x/y) = (n*y)/(d*x), cross-cancelled
    ...
    public Fraction negate() {
        return n.signum() == 0 ? ZERO : new Fraction(n.negate(), d);
    }
    ...
    @Override
    public int hashCode() {
        return 31 * n.hashCode() + d.hashCode();
    }
